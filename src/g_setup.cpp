...
void GetViewTiles(SimView* view)
{
	...
	{
		sprintf(name, "%s/images/tiles.xpm", HomeDir);
		...
		case X_Mem_View:
			...
			{
				if (XpmReadFileToImage(view->x->dpy, name, &view->x->big_tile_image, NULL, &attributes) < 0)
				{
					fprintf(stderr, "Uh oh, Micropolis couldn't read the pixmap file \"%s\".\n", name);
					sim_exit(1); // Just sets tkMustExit and ExitReturn
					return;
		...
		case X_Wire_View:
			...
			{
				if (XpmReadFileToPixmap(view->x->dpy, RootWindowOfScreen(view->x->screen), name, &view->x->big_tile_pixmap, NULL, &attributes) < 0)
				{
					fprintf(stderr, "Uh oh, Micropolis couldn't read the pixmap file \"%s\".\n", name);
					sim_exit(1); // Just sets tkMustExit and ExitReturn
					return;
	...
	else if (view->viewClass == Map_Class)
	{
		if (view->x->small_tile_image == NULL)
		{
			if (view->x->color)
			{
				...
				if (XpmReadFileToImage(view->x->dpy, name, &view->x->small_tile_image, NULL, &attributes) < 0)
				{
					fprintf(stderr, "Uh oh, Micropolis couldn't read the pixmap file \"%s\".\n", name);
					sim_exit(1); // Just sets tkMustExit and ExitReturn
					return;
			...
			{
				view->x->small_tile_image = XCreateImage(view->x->dpy, view->x->visual, 8, ZPixmap, 0, (char*)MickGetHexa(SIM_GSMTILE), 4, 3 * TILE_COUNT, 8, 4);
			}
		}
